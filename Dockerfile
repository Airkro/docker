...
RUN apt-get update && apt-get install --no-install-recommends --fix-missing -y zip && apt-get clean && rm -rf /var/lib/apt/lists/* && npx playwright install && npm cache clean --force

FROM mcr.microsoft.com/playwright:focal as browser
RUN apt-get update && apt-get install --no-install-recommends --fix-missing -y zip fonts-roboto fonts-noto-cjk && apt-get clean && rm -rf /var/lib/apt/lists/* && npx playwright install && npm cache clean --force