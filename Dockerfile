...
RUN apt-get update && apt-get install --no-install-recommends --fix-missing -y git zip autoconf && apt-get clean && rm -rf /var/lib/apt/lists/*

FROM node:lts-slim as python3
RUN apt-get update && apt-get install --no-install-recommends --fix-missing -y git zip autoconf python3 python3-pip && apt-get clean && rm -rf /var/lib/apt/lists/*
